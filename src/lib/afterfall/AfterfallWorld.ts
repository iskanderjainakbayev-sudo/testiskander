...
import { makeEnemy, makeLoot, type AfterfallEnemy, type AfterfallLoot } from './entities';
...
export class AfterfallWorld {
  ...
  private frame = 0; private lastFrame = 0; private elapsed = 0; private publishedAt = 0;
  private yaw = 0; private pitch = 0; private running = false; private reloadingUntil = 0; private lastShot = 0;
  ...
  dispose() { cancelAnimationFrame(this.frame); this.unbind(); this.audio.dispose(); this.renderer.dispose(); }
  ...
  private update(delta: number) {
    this.elapsed += delta; this.updatePlayer(delta); this.updateSurvival(delta); this.updateWorld(delta); this.updateEnemies(delta); this.updateWeather(delta);
  ...
  private updateEnemies(delta: number) {
    for (const enemy of this.enemies) { if (!enemy.alive) continue; const distance = enemy.mesh.position.distanceTo(this.camera.position); if (distance < 27) { const direction = this.camera.position.clone().sub(enemy.mesh.position); direction.y = 0; if (distance > 1.6) enemy.mesh.position.addScaledVector(direction.normalize(), enemy.speed * delta); enemy.mesh.rotation.y = Math.atan2(direction.x, direction.z); }
