...
export function CreatureCard({ species }: { species: Species }) {
  ...
  return (
    <article className="creature-card" style={{ '--species-color': color } as React.CSSProperties}>
      ...
      <dl>
        <div><dt>BIOME</dt><dd>{species.preferredBiome}</dd></div>
        <div><dt>DEPTH</dt><dd>{species.depthRange}</dd></div>
        <div><dt>DIET</dt><dd>{species.diet}</dd></div>
        <div><dt>AVERAGE SIZE</dt><dd>{species.averageSize}</dd></div>
        <div><dt>PERSONALITY</dt><dd>{species.personality}</dd></div>
        <div><dt>ECOSYSTEM ROLE</dt><dd>{species.ecosystemRole}</dd></div>
        <div><dt>SPECIAL ABILITY</dt><dd>{species.specialAbility}</dd></div>
        ...
        <div><dt>CALL</dt><dd>{species.originalSounds.join(' · ')}</dd></div>
        <div><dt>PALETTE</dt><dd>{species.palette.map((tone) => `#${tone.toString(16).padStart(6, '0')}`).join(' · ')}</dd></div>
        <div><dt>LORE</dt><dd>{species.loreDescription}</dd></div>
      </dl>
    </article>
  );
}
